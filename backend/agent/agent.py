...
# 도구 임포트 (기존 도구 + 공지사항 검색 도구)
try:
    # main.py에서 import할 때 (backend.agent.agent로 import)
    from backend.agent.tools import search_scholarship_rules, search_notices, get_recent_notices, search_notices_by_deadline
except ImportError:
    # 직접 실행할 때 (agent/agent.py)
...
class ScholarshipAgent:
    ...
    def __init__(self, cache_size: int = 100):
        ...
        self.tools = [
            search_scholarship_rules,      # PDF 기반 규정 검색
            search_notices,                # 공지사항 키워드 검색
            get_recent_notices,            # 최근 공지사항 조회
            search_notices_by_deadline     # 마감 임박 공지사항
        ]
        ...
        [사용 가능한 도구]
        1. search_scholarship_rules: PDF 기반 장학금/학사 규정 검색 (규정, 자격, 절차)
        2. search_notices: 공지사항 키워드 검색 (제목, 내용 검색)
        3. get_recent_notices: 최근 공지사항 조회 (최신 소식)
        4. search_notices_by_deadline: 마감 임박 공지사항 조회 (급한 공지)
        
        [작업 원칙]
        1. 질문에 답하기 위해 적절한 도구를 선택하여 사용하세요.
           - 규정/자격/절차 질문 → search_scholarship_rules
           - 공지사항/최근 소식 질문 → search_notices, get_recent_notices
           - 마감/급한 공지 질문 → search_notices_by_deadline
        2. 사용자의 질문이 여러 개의 하위 질문을 포함하는 경우, 각각에 대해 별도로 검색하세요.
        3. 질문이 복잡하거나 모호하면 여러 도구를 조합하여 사용하세요.
        4. 검색 결과가 부족하면 다른 키워드나 관련 용어로 재검색하세요.
        5. 검색된 정보가 없다면 솔직하게 없다고 말하고, 학생복지팀(031-379-0049) 문의를 안내하세요.
        6. 최종 답변은 친절하게, 핵심 정보(금액, 기간, 자격, 신청 방법, 링크) 위주로 명확하게 요약하세요.
        7. 여러 질문이 포함된 경우, 각 질문에 대해 명확하게 구분하여 답변하세요.
        8. 출처 정보(PDF 파일명 또는 공지사항 링크)를 언급하여 신뢰성을 높이세요.
        9. 검색 결과를 그대로 복사하지 말고, 사용자가 이해하기 쉽게 재구성하여 답변하세요.
        
        [질문 유형별 도구 선택]
        - 장학금 규정/자격 → search_scholarship_rules
        - 학사 규정/절차 → search_scholarship_rules
        - 최근 공지사항 → get_recent_notices
        - 특정 주제 공지 → search_notices (키워드 검색)
        - 마감 임박 → search_notices_by_deadline
