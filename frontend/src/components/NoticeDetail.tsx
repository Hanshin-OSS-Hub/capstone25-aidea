...
import { X, Calendar, FileText, Download, ExternalLink, Sparkles, Bookmark, BookmarkCheck, Loader2 } from 'lucide-react';
...
interface NoticeDetailProps {
  ...
  onGenerateSummary?: (noticeId: number) => Promise<void>;
}
...
export function NoticeDetail({
  ...
  const hasNoSummary = !notice.detailContent?.summary?.핵심안내 || notice.detailContent.summary.핵심안내 === 'AI 요약 정보가 없습니다.';

  const handleGenerateSummary = async () => {
    if (!onGenerateSummary || isGenerating) return;
    setIsGenerating(true);
    try {
      await onGenerateSummary(notice.id);
    } catch (err) {
        ...
        <div className="px-6 py-6">
          ...
          <div className="bg-blue-50 rounded-lg p-4 mb-6">
            <div className="flex items-center justify-between mb-3">
              ...
              {hasNoSummary && onGenerateSummary && (
                <button
                  onClick={(e) => { e.stopPropagation(); handleGenerateSummary(); }}
                  disabled={isGenerating}
                  className="flex items-center gap-2 px-3 py-1.5 bg-blue-600 text-white text-sm font-medium rounded-lg hover:bg-blue-700 disabled:opacity-60 disabled:cursor-not-allowed"
                >
                  {isGenerating ? (
                    <>
          ...
          <div className="flex space-x-3">
            {notice.detailContent?.originalLink && notice.detailContent.originalLink !== '#' ? (
              <a
              ...
              <div
                className="flex-1 flex items-center justify-center space-x-2 px-6 py-3 bg-gray-200 text-gray-500 rounded-lg cursor-not-allowed font-medium"
                title="원문 링크가 등록되지 않았습니다"
              >
                <ExternalLink className="w-5 h-5" />
                <span>원문 보기 (링크 없음)</span>
              </div>
            )}
            <button
