...
export function BookmarkedNotices({ bookmarkedNotices, onNoticeClick, onBookmarkToggle }: BookmarkedNoticesProps = {}) {
  ...
  // API 데이터가 있으면 사용 (빈 배열이어도 빈 화면 표시), 없으면 목업
  const notices = bookmarkedNotices !== undefined && bookmarkedNotices !== null
    ? bookmarkedNotices
        ...
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
          {sortedNotices.map(notice => (
            <OpportunityCard
              ...
              isBookmarked={true}
              onBookmarkToggle={onBookmarkToggle ? () => onBookmarkToggle(notice.id) : undefined}
            />
          ))}
        </div>
      ) : (
