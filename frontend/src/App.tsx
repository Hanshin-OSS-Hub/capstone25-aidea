...
import { getNoticeDetail, analyzeNotice } from './lib/api-functions';
...
export default function App() {
  ...
  // 공지사항 상세 데이터 로드 (API 실패 시 목업/opportunities에서 fallback)
  const loadNoticeDetail = async (noticeId: number, fallbackOpportunity?: any) => {
    try {
      const response = await getNoticeDetail(noticeId, 1);
      if (response.success) {
        const apiNotice = response.data?.data ?? response.data;
        ...
        const hasAiSummary = ai?.status === 'success' && summaryText.length > 0;

        // AI 요약 없으면 자동 생성 시도
        let finalNotice = apiNotice;
        let finalAi = ai;
        let finalSummaryText = summaryText;
        if (!hasAiSummary) {
          try {
            const analyzeRes = await analyzeNotice(noticeId);
            const refreshed = analyzeRes?.data ?? analyzeRes;
            if (refreshed?.notice_id) {
              finalNotice = refreshed;
              finalAi = refreshed.ai;
              const refSummary = Array.isArray(finalAi?.summary_3lines) ? finalAi.summary_3lines : [];
              finalSummaryText = refSummary.filter(Boolean).join(' ');
            }
          } catch (_) {
            // 자동 생성 실패 시 기존 데이터로 표시
          }
        }

        const extracted = finalAi?.extracted_json || {};
        const category = normalizeCategory(finalAi?.category || finalNotice?.tags?.[0]);
        const finalEndDate = finalAi?.end_date || endDate;
        const finalStartDate = finalAi?.start_date || startDate;

        const viewModel = {
          id: finalNotice.notice_id,
          title: finalNotice.title,
          category,
          deadline: finalEndDate || '',
          daysLeft: finalEndDate ? calculateDaysLeft(finalEndDate) : 0,
          description: finalSummaryText || (finalNotice.content ? String(finalNotice.content).slice(0, 140) + '...' : ''),
          isNew: false,
          department: '',
          publishDate: finalEndDate || '',
          detailContent: {
            summary: {
              일정: finalStartDate && finalEndDate ? `${finalStartDate} ~ ${finalEndDate}` : (finalEndDate ? `~ ${finalEndDate}` : '일정 정보 없음'),
              시간: '',
              장소: extracted.location || '',
              대상: extracted.target || '',
              핵심안내: finalSummaryText || 'AI 요약 정보가 없습니다.',
            },
            fullContent: finalNotice.content || '',
            keywords: [],
            attachments: [],
            originalLink: finalNotice.original_url || '',
          },
        };
      ...
      // API 404 등 실패 시: 목업/opportunities 데이터로 상세 표시
      const opp = fallbackOpportunity;
      if (opp) {
        const baseDetail = {
          summary: {
            일정: opp.deadline ? `~ ${opp.deadline}` : '일정 정보 없음',
            시간: '',
            장소: '',
            대상: '',
            핵심안내: opp.description || '상세 정보를 불러오지 못했습니다.',
          },
          fullContent: opp.description || '',
          keywords: opp.detailContent?.keywords || [],
          attachments: opp.detailContent?.attachments || [],
          originalLink: opp.detailContent?.originalLink || '#',
        };
        const viewModel = {
          id: opp.id,
          title: opp.title,
          ...
          daysLeft: typeof opp.daysLeft === 'number' ? opp.daysLeft : calculateDaysLeft(opp.deadline || ''),
          description: opp.description || '',
          isNew: opp.isNew || false,
          department: opp.department || '',
          publishDate: opp.publishDate || opp.deadline || '',
          detailContent: { ...baseDetail, ...(opp.detailContent || {}) },
        };
        setSelectedNotice(viewModel);
        setShowNoticeDetail(true);
      } else {
  ...
  const mockOpportunities = [
    ...
    },
    {
      ...
      detailContent: {
        ...
        originalLink: 'https://www.hanshin.ac.kr/bbs/hanshin/73/artclList.do',
      },
    },
    {
      id: 4,
      ...
      detailContent: {
        ...
        originalLink: 'https://www.hanshin.ac.kr/bbs/hanshin/72/artclList.do',
      },
    },
    {
      id: 5,
      ...
      detailContent: {
        ...
        originalLink: 'https://www.startup.go.kr/',
      },
    },
    {
      id: 6,
      ...
      detailContent: {
        ...
        originalLink: 'https://www.hanshin.ac.kr/bbs/hanshin/74/artclList.do',
      },
    },
    // 한신대학교 관련 공지사항
    {
    ...
    // 추가 일반 공지사항 5개
    {
      ...
      detailContent: {
        ...
        originalLink: 'https://www.work.go.kr/',
      },
    },
    {
      id: 11,
      ...
      detailContent: {
        ...
        originalLink: 'https://www.hanshin.ac.kr/bbs/hanshin/75/artclList.do',
      },
    },
    {
      id: 12,
      ...
      detailContent: {
        ...
        originalLink: 'https://www.hanshin.ac.kr/bbs/hanshin/76/artclList.do',
      },
    },
    {
      id: 13,
      ...
      detailContent: {
        ...
        originalLink: 'https://www.hanshin.ac.kr/bbs/hanshin/77/artclList.do',
      },
    },
    {
      id: 14,
      ...
      detailContent: {
        ...
        originalLink: 'https://www.1365.go.kr/',
      },
    },
  ];
  ...
  const opportunities = notices.length > 0
    ? notices.map(notice => ({
        ...
        publishDate: '',
        tags: notice.tags,
        detailContent: notice.original_url ? { originalLink: notice.original_url } : null,
      }))
    : mockOpportunities;
  ...
  const handleNoticeClick = async (id: number) => {
    ...
    await loadNoticeDetail(id, opp);
  };

  const handleGenerateSummary = async (id: number) => {
    await analyzeNotice(id);
    const opp = opportunities.find(o => o.id === id);
    await loadNoticeDetail(id, opp);
  };
  ...
  const handleBookmarkToggle = async (id: number) => {
    const opp = opportunities.find(o => o.id === id);
    await toggleFavorite(id, opp ? { title: opp.title, category: opp.category, end_date: opp.deadline, daysLeft: opp.daysLeft } : undefined);
  };
          ...
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
            {finalFilteredNotices.map(opportunity => (
              <OpportunityCard
                ...
                isBookmarked={bookmarkedNotices.includes(opportunity.id)}
                onBookmarkToggle={() => handleBookmarkToggle(opportunity.id)}
              />
            ))}
          </div>
      ...
      <div className="min-h-screen bg-gray-50 pb-20">
        ...
        <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          <MyPage 
            ...
            onBookmarkedNoticeClick={(id) => handleNoticeClick(id)}
            participatedCount={participatedNotices.length}
            bookmarkedNotices={favoriteNotices}
